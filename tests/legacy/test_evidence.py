...
def test_exit_semantics_and_raw_stderr(tmp_path):
 ...
 packet=json.loads(p.stdout);assert packet['exit_code']==7 and Store(tmp_path/'s').retrieve(packet['receipt'],'stderr')['text']=='failed\n'
...
def test_timeout_keeps_partial_output_and_failure(tmp_path):
 ...
 assert p['timed_out'] and p['exit_code']<0 and s.retrieve(p['receipt'])['text']=='partial\n'
...
def test_bounded_packet_retains_retrieval_and_watched_changes(tmp_path):
 ...
 p=run(s,[sys.executable,'-c','from pathlib import Path;Path("a").write_text("new");print("ERROR "+"x"*20000)'],tmp_path,'test',watch=['a'])
...
def test_read_amplification_exposed_not_hidden(tmp_path):
 ...
 assert got['bytes']==5 and got['io']['object_bytes_read']>=50000
